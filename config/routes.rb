  ...
  Rails.application.routes.draw do
    ...
    scope "(:locale)", locale: /#{I18n.available_locales.join("|")}/ do
      ...
      if ArticleCategory.table_exists?
        get "/:root_category(/sort=:sort)(/tags=:tags)(/page=:page)/(*url)", to: "articles#smart_article", as: :smart_article, root_category: /#{ arr = ArticleCategory.roots.published.map(&:translations); translations = []; arr.each {|sub_arr| translations.concat sub_arr };   translations.map(&:url_fragment).select{|s| s.present? }.uniq.join('|')}/
      end
